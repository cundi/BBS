...
from bb.models import TopicReadTracker, ForumReadTracker, Topic, Post
...
@register.tag
def bb_time(parser, token):
    try:
        tag, context_time = token.split_contents()
    except ValueError:
        raise template.TemplateSyntaxError('bb_time requires single argument')
    else:
        return bbTimeNode(context_time)


class bbTimeNode(template.Node):
...
@register.filter
def bb_topic_moderated_by(topic, user):
    """
    Check if user is moderator of topic's forum.
    """
    warnings.warn("bb_topic_moderated_by filter is deprecated and will be removed in later releases. "
                  "Use bb_may_moderate_topic(user, topic) filter instead",
                  DeprecationWarning)
    return perms.may_moderate_topic(user, topic)


@register.filter
def bb_editable_by(post, user):
    """
    Check if the post could be edited by the user.
    """
    warnings.warn("bb_editable_by filter is deprecated and will be removed in later releases. "
                  "Use bb_may_edit_post(user, post) filter instead",
                  DeprecationWarning)
    return perms.may_edit_post(user, post)


@register.filter
def bb_posted_by(post, user):
    """
...
@register.filter
def bb_is_topic_unread(topic, user):
...
@register.filter
def bb_topic_unread(topics, user):
    """
    对请求用户的qs进行unread标记
...
@register.filter
def bb_forum_unread(forums, user):
    """
    ...
    if user.is_authenticated():
        ...
        for mark in forum_marks:
            ...
            if (curr_forum.updated is None) or (curr_forum.updated <= mark.time_stamp):
                if not any((f.unread for f in bb_forum_unread(curr_forum.child_forums.all(), user))):
                    forum_dict[mark.forum.id].unread = False
    return forum_list


@register.filter
def bb_topic_inline_pagination(topic):
    page_count = int(math.ceil(topic.post_count / float(settings.FORUM_TOPIC_PAGE_SIZE)))
    if page_count <= 5:
...
# @register.filter
# def bb_topic_poll_not_voted(topic, user):
...
def bb_get_latest_topics(context, cnt=5, user=None):
...
@register.assignment_tag(takes_context=True)
def bb_get_latest_posts(context, cnt=5, user=None):
    ...
    for method in inspect.getmembers(perms):
        if inspect.ismethod(method[1]) and inspect.getargspec(method[1]).args[0] == 'self' and \
            ...
            if len(inspect.getargspec(method[1]).args) == 3:
                register.filter('%s%s' % ('bb_', method[0]), partial(method[0], perms))
            elif len(inspect.getargspec(method[1]).args) == 2:  # only user should be passed to permission method
                register.filter('%s%s' % ('bb_', method[0]), partial_no_param(method[0], perms))
...
@register.filter
def bbm_calc_topic_views(topic):
    cache_key = utils.build_cache_key('anonymous_topic_views', topic_id=topic.id)
