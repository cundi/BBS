...
class PasswordField(forms.CharField):
    ...
    def clean(self, value):
        ...
        if not num_check or not char_check:
            raise forms.ValidationError(u'密码至少包含一个数字和一个字母，最少长度7位.')

        return super(PasswordField, self).clean(value)


class RegistrationAskForm(forms.ModelForm):
    username = forms.RegexField(label=_("Username"), max_length=30, regex=r'^\w+$',
                                help_text=_("Required. max 30 character or fewer. Alphanumeric characters only"),
    ...
    def clean_email(self):
        ...
        if User.objects.filter(email__iexact=self.cleaned_data['email']).count():
            raise forms.ValidationError(_(u"This email address is already in use. Please supply a different one."))
    ...
    def save(self, commit=True, domain_override=None,
             subject_template_name='registration/register_ask_subject.txt',
             email_template_name='registration/register_ask_email.html',
        ...
        user.
        """
        user = super(RegistrationAskForm, self).save(commit=True)
...
class AuthenticationForm(forms.Form):
    ...
    username = forms.CharField(max_length=254, widget=forms.TextInput(
        attrs={'class': 'form-control', 'placeholder': '请输入用户名', }), required=True)
    password = forms.CharField(widget=forms.PasswordInput(
        attrs={'class': 'form-control', 'placeholder': '请输入密码'}), required=True)

    error_messages = {
        'invalid_login': _("Please enter a correct %(username)s and password. "
                           "Note that both fields may be case-sensitive."),
        'inactive': _("This bbs_account is inactive."),
    }
...
class ProfileForm(forms.Form):
    Content = forms.CharField(label=u'content',
                              widget=UEditorWidget({'width': 650, 'height': 110, 'imagePath': 'images/',
                                                    'filePath': 'files/'})
                              )