...
class DefaultPermissionHandler(object):
    ...
    def may_view_topic(self, user, topic):
        ...
        if topic.on_manage:
            return user.is_authenticated() and (user == topic.user or user in topic.forum.manager)
        return True

    def may_manage_topic(self, user, topic):
        return user.is_superuser or user in topic.forum.manager.all()

    def may_close_topic(self, user, topic):
        """ return True if `user` may close `topic` """
        return self.may_manage_topic(user, topic)
    ...
    def may_create_post(self, user, topic):
        ...
        return settings.FORUM_ENABLE_ANONYMOUS_POST or user.has_perm('pybb.add_post')
    ...
    def may_subscribe_topic(self, user, forum):
        ...
        return not settings.FORUM_DISABLE_SUBSCRIPTIONS
    ...
    def filter_posts(self, user, qs):
        ...
        if not settings.FORUM_PREMODERATION or user.is_superuser:
    ...
    def may_attach_files(self, user):
        ...
        """
        return settings.FORUM_ATTACHMENT_ENABLE
...
perms = utils.resolve_class(settings.FORUM_PERMISSION_HANDLER)
